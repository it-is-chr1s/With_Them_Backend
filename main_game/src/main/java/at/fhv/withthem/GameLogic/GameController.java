...
public class GameController {
    ...
    public ResponseEntity<Boolean> getGame(@PathVariable String gameId, @PathVariable String playerName) {
        ...
        ;
        if (gameService.getGame(gameId).getPlayers().size() >= gameService.getGame(gameId).getSettings()
                .getMaxPlayers()) {
    ...
    public ResponseEntity<String> setSettings(@PathVariable String gameId, @PathVariable String playerName,
            @PathVariable String settings, @PathVariable String value) {
    ...
    public void loadSabotages(String lobbyID, List<TaskPosition> sabotagePositions) {
        ...
        for (TaskPosition sabotagePosition : sabotagePositions) {
            initTaskMessages
                    .add(new InitTaskMessage(lobbyID, sabotagePosition.getTaskType(), sabotagePosition.getId()));
        }
        ObjectMapper mapper = new ObjectMapper();
        try {
    ...
    public PlayersRequest getPlayers(@PathVariable String gameId) {
        ...
        List<PlayerInfo> alivePlayerInfo = alivePlayers.stream()
                .map(player -> new PlayerInfo(player.getId(), player.getColor().getHexValue()))
                .collect(Collectors.toList());

        return new PlayersRequest(alivePlayerInfo,
                deadPlayers.stream().map(Player::getId).collect(Collectors.toList()));
    }
    ...
    public void handleKill(KillRequest killRequest) {
        ...
        /*
         * if (success) {
         * messagingTemplate.convertAndSend("/topic/" + gameId + "/kill", "Player " +
         * killerId + " made a kill");
         * } else {
         * messagingTemplate.convertAndSend("/topic/" + gameId + "/killFailed",
         * "Kill failed for player " + killerId);
         * }
         */
    }
    ...
    public void loadEmergencyMeeting(String gameId, ConcurrentHashMap<String, Player> players) {
        ...
        LoadEmergencyMeetingMessage loadEmergencyMeetingMessages = new LoadEmergencyMeetingMessage(gameId,
                alivePlayers);
        ObjectMapper mapper = new ObjectMapper();
        try {
            System.out
                    .println("Message to EmergencyMeeting:" + mapper.writeValueAsString(loadEmergencyMeetingMessages));
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
